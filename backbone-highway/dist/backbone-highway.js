(function (global, factory) {
  ...
  var trigger = {
    send: function send (routeName, events, args, path) {
      if (!_.isArray(events)) {
  ...
  Route.prototype = {
    ...
    getActionWrapper: function getActionWrapper () {
      ...
      return function actionWrapper () {
        ...
        if (events) trigger.send(name, events, args, path)
  ...
  var highway = {
    ...
    go: function go (to) {
      ...
      if(to.bypass == true &&  to.path) {
         return this.router.navigate(to.path, true);
      }
